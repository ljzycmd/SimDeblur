...
class Centerlize:
    ...
    def __call__(self, img1, img2=None):
        ...
        if img2 is not None:
            img2 = img2.astype(np.float)
            return img1 * 2 - self.pixel_max, img2 * 2 - self.pixel_max
        else:
            return img1 * 2 - self.pixel_max
...
def augment(img1, img2, cfg):
    ...
    if hasattr(cfg, "Centerlize"):
        transforms.append(Normalize(**cfg["Centerlize"]))
